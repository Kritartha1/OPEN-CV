...
class FaceRecognition:
    ...
    def read_db(self, databases_path):
        ...
        for folder in os.listdir(databases_path):
            # for file in os.listdir(f"{databases_path}/{folder}"):
    ...
    while True:
        ...
        if msgs is not None:
            ...
            for i, detection in enumerate(dets):
                ...
                # cv2.rectangle(frame, (bbox[0],h-bbox[1]), (bbox[2],h-bbox[3]), (10, 245, 10), 2)
               ...
               # text.putText(frame, f"{name} {(100*conf):.0f}%", (bbox[0] + 10,h-(bbox[3] + 35)))
                text.putText(frame, f"{fname} {(100*conf):.0f}%", (bbox[0] + 10,h-(bbox[3] + 35)))

                if  name!='unknown' and  id%10==0 and size<12:
                    # img=cv2.flip(img,0)
                    
                    blank = np.zeros(img2.shape[:2], dtype='uint8')
                    rectangle = cv2.rectangle(blank.copy(),  (bbox[0]-9,h-bbox[1]+9), (bbox[2]+9,h-bbox[3]-9), 255, -1)
